...
@contextmanager
def open_zstd_text_writer(path: Path, level: int = ZSTD_LEVEL):
    path.parent.mkdir(parents=True, exist_ok=True)
    proc = subprocess.Popen(
        ["zstd", "-q", "-f", f"-{level}", "-T0", "-o", str(path)],
        stdin=subprocess.PIPE,
    )
    if proc.stdin is None:
        raise RuntimeError(f"Failed to open zstd stdin for {path}")
...
def write_articles(path: Path, rows: Iterable[dict]) -> Counter:
    counts = Counter()
    with open_zstd_text_writer(path) as f:
...
def iter_csv_rows_from_zst(path: Path) -> Iterable[dict]:
    ...
    try:
        ...
        return_code = proc.wait()
        if return_code != 0:
            raise RuntimeError(f"zstd decode failed for {path} with exit code {return_code}")


def dedupe_rows(rows: Iterable[dict], output_path: Path) -> Counter:
    seen: dict[str, str] = {}
    duplicate_groups: dict[str, str] = {}
    group_idx = 0
    counts = Counter()
    with open_zstd_text_writer(output_path) as dst:
...
def write_merged_and_deduped(
    ...
    with open_zstd_text_writer(merged_path) as merged_f, open_zstd_text_writer(deduped_path) as deduped_f:
        merged_writer = csv.DictWriter(merged_f, fieldnames=FIELDNAMES)
        deduped_writer = csv.DictWriter(deduped_f, fieldnames=FIELDNAMES)
        merged_writer.writeheader()
        deduped_writer.writeheader()
        for row in rows:
            merged_writer.writerow(row)
            merged_counts["rows"] += 1
            merged_counts[f"source:{row['dataset_source']}"] += 1
            merged_counts[f"year:{row['publication_date'][:4]}"] += 1
            merged_counts[f"category:{row['category_harmonised']}"] += 1
            ...
            if text_hash in seen:
                if text_hash not in duplicate_groups:
                    group_idx += 1
                    duplicate_groups[text_hash] = f"dup_{group_idx:09d}"
                output_row["is_duplicate"] = "true"
                output_row["duplicate_group_id"] = duplicate_groups[text_hash]
                dedupe_counts["duplicate_rows"] += 1
            else:
                seen[text_hash] = row["article_id"]
            deduped_writer.writerow(output_row)
            dedupe_counts["rows"] += 1

    dedupe_counts["unique_text_hashes"] = len(seen)
    dedupe_counts["duplicate_groups"] = len(duplicate_groups)
    return merged_counts, dedupe_counts
...
def main() -> None:
    ...
    potrika_path = compressed_path(OUT_DIR / "potrika_articles_canonical.csv")
    bnad_path = compressed_path(OUT_DIR / "bnad_articles_canonical.csv")
    merged_path = compressed_path(OUT_DIR / "beni_unified_articles.csv")
    deduped_path = compressed_path(OUT_DIR / "beni_unified_articles_deduped.csv")
    ...
    def merged_rows() -> Iterable[dict]:
        yield from iter_potrika_articles(args.min_chars)
        yield from iter_bnad_articles(bnad_dir, args.min_chars)

    merged_counts, dedupe_counts = write_merged_and_deduped(merged_path, deduped_path, merged_rows())
