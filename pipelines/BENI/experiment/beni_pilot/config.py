...
class ExperimentConfig(BaseExperimentConfig):
    """BENI experiment configuration.

    Inherits shared fields (seed, max_features, min_df, ngram_range,
    max_iter, output_dir, model_dir, report_dir) from
    BaseExperimentConfig.  Adds BENI-specific paths and hyperparameters.
    """

    # Override BaseExperimentConfig defaults for BENI's directory layout
    output_dir: Path = ROOT / "outputs"
    model_dir: Path = ROOT / "outputs" / "models"
    report_dir: Path = ROOT / "outputs" / "reports"
    data_dir: Path = ROOT.parent / "data"  # up one level: pipelines/data/
    ...
    # --- BENI-specific TF-IDF hyperparameters ---
