#!/usr/bin/env python3
"""
Audio Annotation Lab — Adjudication Script

Resolve disagreements between multiple LLM annotators or human annotators
on audio segment annotations.  Produces a locked reference set of
...
Deliverable:
    - Locked reference set with adjudicated labels per segment
...
def load_annotations(input_dir: str) -> list[dict]:
    ...
    Returns:
        List of records with ``segment_id``, ``annotator_id``,
...
def group_by_segment(annotations: list[dict]) -> dict[str, list[dict]]:
    """Group annotation records by segment ID.

    Args:
        annotations: List of annotation records.

    Returns:
        Dictionary mapping ``segment_id`` → list of records.
    """
    grouped: dict[str, list[dict]] = defaultdict(list)
    for ann in annotations:
        grouped[ann["segment_id"]].append(ann)
    return dict(grouped)
...
def compute_interrater_agreement(
    ...
    Args:
        grouped: Dictionary mapping ``segment_id`` → list of records.
    ...
    for field in fields:
        # Build rating matrix: one row per segment, one column per rater
...
def majority_vote(annotations: list[dict]) -> dict[str, Any]:
    ...
    Args:
        annotations: Annotation records for a single segment.
...
def adjudicate_segment(
    ...
    """Adjudicate annotations for a single segment using the given method.

    Args:
        annotations: Annotation records for a single segment.
        method: One of ``"majority"``, ``"confidence"``, or
                ``"human_review"``.

    Returns:
        Adjudication result dict with ``adjudicated_labels``,
        ``agreement`` info, and ``method``.
    """
    if method == "majority":
        return majority_vote(annotations)
    elif method == "confidence":
        return confidence_weighted(annotations)
    elif method == "human_review":
        result = majority_vote(annotations)
        result["needs_human_review"] = True
        return result
    else:
        logger.warning("Unknown method '%s', falling back to majority", method)
        return majority_vote(annotations)
...
def main() -> None:
    ...
    grouped = group_by_segment(annotations)
    logger.info("Grouped into %d segments", len(grouped))
    ...
    logger.info("Reference set saved: %s (%d segments)", refset_file, len(refset))
    ...
    if n_flagged:
        logger.info(
            "%d segments flagged for human review",
            n_flagged,
        )

    logger.info(
        "Adjudication complete. %d segments in locked reference set.",
        len(refset),
    )
