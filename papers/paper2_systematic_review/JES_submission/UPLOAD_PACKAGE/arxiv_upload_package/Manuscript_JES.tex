...
%           A Systematic Review of Methodologies, Applications, and
%           Research Gaps (2007--2025)
%  Author: Ann Naser Nabil
%  Affil.: Department of Economics, Jahangirnagar University
%  License: CC BY 4.0
%  Preprint submitted to the Journal of Economic Surveys
%  Also deposited on arXiv.org for open access
%  Primary classification: econ.EM (Econometrics)
%  Secondary classifications: q-fin.ST (Statistical Finance),
...
\usepackage{silence}
\WarningFilter{epstopdf}{Shell escape feature is not enabled}
\usepackage{amsmath,amssymb}
\usepackage{graphicx,booktabs,float,multirow,setspace,fancyhdr}
...
\hypersetup{%
  pdfauthor   = {Ann Naser Nabil},
  pdftitle    = {Economic Narrative Indices and Media-Based Sentiment Measures:
                  A Systematic Review of Methodologies, Applications, and
                  Research Gaps (2007--2025)},
  pdfsubject  = {Economics - Econometrics},
  pdfkeywords = {sentiment analysis, economic narratives, systematic review,
                 natural language processing, forecasting, validation,
                 meta-analysis, geographic gaps}
}
...
\author{\textbf{Ann Naser Nabil}\orcidlink{0009-0006-3561-045X}\\[4pt]
  \small Department of Economics, Jahangirnagar University,\\
  \small Savar, Dhaka 1342, Bangladesh\\
  \small\texttt{ann.n.nabil@gmail.com}}
\date{\today}
...
\noindent\small\textit{This paper is a preprint submitted to the}
\textit{Journal of Economic Surveys}
\textit{for consideration.}
\textit{An earlier version of this work is also deposited on arXiv.org.}
\par\vspace{0.5em}

\hrule\vspace{1em}

\begin{abstract}
We review 46 empirical studies (2007--2025) that extract sentiment from text for economic forecasting, plus 20 theoretical papers on sentiment in macroeconomic models. Sentiment-based models improve forecast accuracy by 12--20\% (median RMSE reduction), but gains depend on baseline strength: ~12--15\% against strong benchmarks (professional forecasts, ARIMA) versus larger but less realistic gains against weak baselines (AR(1)). Publication bias further tempers the headline figure. Methods have evolved from dictionaries (55\% of studies) through ML and transformers to LLMs (3\%). While 83\% of studies use out-of-sample testing, only 33\% follow real-time data protocols. Geographic and linguistic concentration is severe: 56\% of studies focus on North America, 84\% on English text, and zero indices exist for Africa, Latin America, or Bangla-speaking regions (265 million speakers). We develop a validation framework and propose expansion strategies, using the Bangla Economic Narrative Index (BENI) as a roadmap.
\end{abstract}

\noindent \textbf{Keywords:} Sentiment analysis; economic narratives; systematic review; natural language processing; forecasting; validation frameworks \\
...
Despite 18 years of research and methodological sophistication, critical gaps remain. \textit{Geographic bias is stark}: 56\% of studies focus on North America, 20\% on Europe, and coverage of Africa and Latin America is absent in the reviewed sample. \textit{Linguistic coverage is narrow}: 84\% analyze English-language text; no studies are identified that construct economic sentiment indices for Bangla (265M speakers), Hindi (600M), or most non-OECD languages. \textit{Theory is weak}: while narrative economics increasingly emphasizes sentiment transmission, empirical sentiment research remains descriptive rather than mechanistic.

This systematic review addresses four questions: (1) What methodologies have been employed? (2) Where does sentiment add forecasting value? (3) How robust are current measures? (4) What are critical research gaps? It synthesizes 46 empirical studies (plus 20 theoretical papers discussed separately in Section~3) to establish: (i) comprehensive methodological taxonomy; (ii) quantitative synthesis of forecast improvements; (iii) explicit documentation of geographic/linguistic gaps; (iv) proposal for expanding sentiment analysis to underrepresented regions (specifically, Bangla-speaking markets).

This paper is closest in spirit to Algaba et al. (2020), whose \textit{Journal of Economic Surveys} article organized the econometric sentiment literature and clarified how sentiment measures are constructed and used across economics and finance. The present review does not seek to replace that survey. Instead, it updates and narrows the question in four ways that are directly relevant for the post-2020 forecasting literature. First, it focuses on whether sentiment measures improve forecasts out of sample, rather than on sentiment measurement in general. Second, it conditions reported forecast gains on benchmark strength, distinguishing comparisons against weak autoregressive baselines from comparisons against ARIMA, professional forecasts, or published nowcasts. Third, it audits validation practices---real-time data discipline, rolling-window design, Diebold-Mariano testing, Model Confidence Sets, and reporting of annotation reliability---because these design choices determine whether reported gains can survive operational forecasting. Fourth, it documents geographic and linguistic concentration, an issue that has become more salient as multilingual transformers and LLMs make non-English sentiment measurement technically feasible. The contribution is therefore not a general overview of sentiment econometrics, but a forecasting-oriented evidence review that asks how much predictive value sentiment adds, under what validation standards, and where the evidence base remains thin.

\section{Methodology}

\subsection{Search and Inclusion}

Following PRISMA guidelines, an initial systematic search was conducted in January 2025 and updated through December 2025 across five databases: Web of Science (Core Collection), Scopus, EconLit (via EBSCO), SSRN, and Federal Reserve working paper series. The search string used across all databases was: (``sentiment analysis'' OR ``text analysis'' OR ``narrative index'') AND (``forecasting'' OR ``economic'' OR ``macroeconomic'') AND (``machine learning'' OR ``NLP'' OR ``natural language''). No date restrictions were applied beyond the field's emergence (2007--2025). Database searches were supplemented with backward citation chasing from key reviews (Algaba et al., 2020; Kearney \& Liu, 2014) and forward citation tracking of Tetlock (2007) via Google Scholar.
...
This review is prepared as a systematic review of the sentiment-based economic forecasting literature and follows the \textit{Journal of Economic Surveys}' expectations for transparent search, screening, and reporting as set out in the MAER-Net reporting guidelines for systematic reviews and meta-analyses in economics (Havr\'anek et al., 2020). The submission includes the search terms, databases searched, search dates, a PRISMA flow diagram, and separate supplementary files containing the extracted study matrix and quality assessment. The search strategy was designed to capture both economics-facing and text-analysis-facing terminology: sentiment analysis, text analysis, narrative index, forecasting, economic, macroeconomic, machine learning, NLP, and natural language. Searches were run across Web of Science, Scopus, EconLit, SSRN, and Federal Reserve working paper series, with backward citation chasing from major prior reviews and forward citation tracking of Tetlock (2007). The final search update covers literature available through December 2025.

This review is a structured quantitative synthesis of reported forecast improvements rather than a formal meta-analysis with pooled effect-size estimation. Because the reviewed studies report heterogeneous targets, horizons, benchmarks, and effect-size metrics, and many lack standard errors, a full random-effects meta-analysis is not feasible with the current evidence base. Practitioners conducting new meta-analyses in this domain are directed to Irsova et al. (2024) for step-by-step guidance on modern meta-analysis methods in social science research, including publication bias correction, heterogeneity exploration, and the use of artificial intelligence tools.

Screening proceeded in three stages. First, duplicate records were removed from the initial database and citation-search results. Second, titles and abstracts were screened for relevance to text-derived sentiment or narrative measurement in economic or financial forecasting. Third, full texts were assessed against the inclusion criteria. Studies were retained only when they extracted sentiment or narratives from text, applied the resulting measure to an economic or financial forecasting problem, and reported some form of validation or empirical assessment. Theoretical papers were retained separately when they developed economic models of sentiment, narratives, or belief-driven fluctuations that help interpret the empirical forecasting literature.

To make the evidence auditable, the supplementary literature matrix codes each paper's primary method, forecast domain, region, language, validation practices, benchmark type, and standardized forecast-performance entry where comparable information is available. Reported percentages in the manuscript are computed from these coded columns. The quality-assessment file contains one scored row per reviewed paper using the stated methodology, data-quality, and validation dimensions.

This reporting structure is intended to separate reproducible descriptive coding from interpretive synthesis. Counts and percentages are therefore traceable to the supplementary files, while judgments about mechanisms, policy relevance, and future research priorities are presented as narrative conclusions from the coded evidence rather than as mechanically estimated meta-analytic effects.
...
Integrating the above elements, this review proposes a framework for understanding sentiment-based forecasting:
...
The empirical sample also spans several more specialized applications. Financial-market and corporate-disclosure studies include H\'ajek and Olej (2013), Kr\"aussl and Mirgorodskaya (2017), Hassan et al. (2019), Gite et al. (2021), Liapis et al. (2023), Talazadeh and Perakovi\'c (2024), Zhang et al. (2024), and Ad\"ammer and Sch\"ussler (2020), the latter showing that news topics can improve equity-premium forecasts. Macroeconomic nowcasting and inflation applications include Bortoli et al. (2018), Huang et al. (2018), Caruso (2019), Ardia et al. (2019), Jones et al. (2020), Clements and Reade (2020), Aguilar et al. (2021), Shapiro et al. (2022), Ashwin et al. (2021), Lukauskas et al. (2022), Jiang et al. (2022), Ellingsen et al. (2022), Thorsrud (2020), Barbaglia et al. (2023), Allard et al. (2024), Audrino and Offner (2024), Eugster and Uhl (2024), Rogmann et al. (2024), Beiranvand et al. (2024), Simionescu and Nicula (2024), Matos (2024), Seo (2025), Weinig and Fritsche (2025), and Kaczmarek et al. (2025). Policy-uncertainty, geopolitical-risk, and institutional communication studies include Baker et al. (2016), Gavaldon (2017), Castle et al. (2017), Choudhary et al. (2020), Aharon et al. (2022), Caldara and Iacoviello (2022), Gardner et al. (2022), Tkacova and Gavurova (2023), Cummins et al. (2024), Garcia (2024), and the San Francisco Federal Reserve Bank (2025). Theoretical and methodological contributions retained for context include Gentzkow et al. (2019), Andre et al. (2021), Korinek (2023), Lange et al. (2022), Bieri (2025), Borja et al. (2024), Gro{\ss}-Klu{\ss}mann (2024), and Andre et al. (2024).
...
\caption{\textit{Summary of Key Metrics: 46 Empirical Papers (N=46) plus 20 Theoretical Papers (N=20).} Theoretical papers develop macroeconomic models of sentiment without computational text extraction. Methodological distribution and validation practices reflect all 66 papers; forecast performance metrics reflect N=46 empirical papers only. Geographic coverage includes both empirical and theoretical papers; region totals need not sum to 66 because some studies have multi-country scope and some theoretical papers are not assigned to a single region. OOS=Out-of-sample; DM=Diebold-Mariano; MCS=Model Confidence Sets; SPA=Superior Predictive Ability. The \textit{Assessment} column uses section-specific scales: (1) Methodological distribution --- research volume per method: Extensive ($\geq 20$), Moderate (10--19), Limited (5--9), Sparse ($<5$); (2) Forecast performance --- evidence strength; (3) Validation practices --- adoption level; (4) Geographic and language coverage --- research density/representation.}
...
\textbf{Large language models} (2 papers, 3\%) employ GPT-3.5, GPT-4, or equivalent models for zero-shot or few-shot sentiment extraction via prompting. Reported LLM classification performance is high in the small number of reviewed applications, but these figures should be interpreted cautiously because the studies use different tasks, datasets, and validation procedures. LLM pre-training corpora may also include financial news and common sentiment benchmarks, creating potential contamination where the model has effectively seen test data during training. LLM deployment faces significant barriers: hallucinations are more consequential than traditional classification errors, inference costs can be material for very large news archives, and model opacity limits explainability required for policy applications. Current evidence from 2 papers is preliminary; LLMs are best suited for exploratory analysis and rapid prototyping rather than production forecasting systems.
...
Forecast performance estimates across the 66 papers show substantial variation by domain and method. Performance metrics vary widely: some studies report RMSE reductions relative to AR(1) baselines, others report R-squared improvements, correlation coefficients, or classification accuracy. To enable synthesis, reported effects are standardized to equivalent RMSE reduction: a 5\% absolute RMSE improvement translates to approximately 8--12\% reduction in mean squared error depending on baseline variability.
...
\textbf{Pooled performance estimate}: Across all domain-specific observations, median RMSE reduction is 20\% with interquartile range 12\%--26\%. Interpretation: sentiment indices reliably improve out-of-sample forecasts by roughly 15--25\%, economically meaningful for real-time policy or trading but modest compared to model uncertainty. The result is robust to domain. Figure~\ref{fig:performance_dist} visualizes the distribution of RMSE reductions across economic domains.

\begin{figure*}[t]
\centering
\caption{Distribution of RMSE Reduction by Economic Domain}
\label{fig:performance_dist}
\begin{tikzpicture}
\begin{axis}[
    width=0.95\textwidth,
    height=0.6\textwidth,
    ylabel=RMSE Reduction (\%), xlabel=,
    xtick={1,2,3,4,5,6,7},
...
\end{tikzpicture}
\raggedright\footnotesize
\textit{Note:} Box plots show the distribution of RMSE reductions reported across studies for each economic domain. Median values: GDP 18\%, Inflation 22\%, Stock Returns 15\%, Recession 20\%, Employment 16\%, Consumer Confidence 25\%, Volatility 28\%. Whiskers extend to the full range of reported values. Based on N=46 empirical papers.
\end{figure*}
...
\footnotesize \textbf{Real-time Protocol Audit}: Approximately 35\% of papers (23/66) are coded as lacking clear real-time data discipline or as using information that would be difficult to observe at the forecast date. This suggests practical effectiveness may be lower than reported, though the median 20\% improvement remains economically meaningful.\normalsize
...
\centering
\small
\caption{\textit{Validation Practices by Period.} Adoption counts and rates for the four main validation practices, split by publication period. The pre-2020 sample (N=19) is composed predominantly of highly cited foundational papers, which all report OOS testing and Diebold-Mariano tests. The 2020-onward sample (N=47) is larger and more heterogeneous, resulting in lower proportional adoption despite a higher mean Validation dimension score (0.89 pre-2020 vs.~1.32 post-2020), reflecting greater rigor among papers that do validate.}
\label{tab:validation_by_era}
...
The geographic distribution of research shows stark concentration: North America hosts 37 papers (56\%), Europe 13 (20\%), East/Southeast Asia 9 (14\%), South Asia 4 (6\%), and the Middle East 1 paper (2\%). The North American category is heavily US-centered but includes papers with Canadian or multi-country North American scope. Zero coverage in Africa and Latin America, and only one Middle Eastern application, represents a profound research gap relative to economic significance. Figure~\ref{fig:geographic} visualizes this concentration.
...
\end{tikzpicture}
\raggedright\footnotesize
\textit{Note:} Bar chart shows number of papers by geographic region (N=66). North America dominates with 37 papers (56\%). Africa and Latin America have zero studies, and the Middle East is represented by one Iran-focused study, indicating a critical research gap. Regional totals need not sum to 66 because some studies have multi-country scope and some theoretical papers are not assigned to a single region.
\end{figure*}
...
Consolidating evidence across 66 papers and best-practice standards in forecasting literature, the review identifies a tier of validation practices:
...
\textbf{Illustrative Roadmap: Bangla Economic Narrative Index (BENI)}
\begin{itemize}
...
\item Potential contribution: A native-language economic sentiment index for a major underrepresented South Asian language
...
\textbf{The Multilingual NLP Opportunity}: Recent advances in multilingual language models dramatically lower barriers to geographic expansion. Models such as multilingual BERT (mBERT), XLM-R (Conneau et al., 2020), and MuRIL cover 100+ languages with zero-shot cross-lingual transfer, enabling sentiment extraction without language-specific training data. For Bangla specifically, BanglaBERT (Bhattacharjee et al., 2022) achieves state-of-the-art performance across 13 NLP tasks. These resources, combined with increasing availability of digitized news corpora in emerging markets, make the geographic expansion agenda technically feasible in ways that were not possible even five years ago.
...
Sentiment analysis has developed from a niche application into a recurring tool in economic forecasting research. This systematic review of 46 empirical studies (supplemented by 20 theoretical papers) yields five principal findings.

\textbf{First}, sentiment-based forecasting often improves out-of-sample accuracy across macroeconomic and financial domains. The unconditional median RMSE reduction is approximately 20\%, but this figure pools comparisons against baselines of markedly different strength. Studies using strong benchmarks (professional forecasts, ARIMA) report improvements of $\sim$12--15\%, while those using weak baselines (AR(1)) report larger but less realistic gains. Publication bias further tempers the headline figure: only a small number of reviewed papers report null or negative results. The realistic range for sentiment value-added, conditional on strong baselines and accounting for publication bias, is likely closer to 10--15\%.

\textbf{Second}, methodological evolution from dictionaries (55\% of empirical studies) through ML and transformers to LLMs has improved representational flexibility but introduces new trade-offs. Dictionaries remain dominant due to interpretability, reproducibility, and zero marginal cost. LLMs report strong classification performance in early applications but face barriers of hallucination risk, inference cost, and opacity that limit production deployment. Accuracy non-comparability across methods, datasets, and tasks means the field lacks a single ``best'' method; the appropriate choice depends on the specific forecasting application and its requirements for interpretability, scalability, and accuracy.

\textbf{Third}, validation practices have improved but remain uneven. Out-of-sample testing (83\% of studies) is now standard, but adoption of best-practice methods remains low: real-time data protocols (33\%), Model Confidence Sets (18\%), and reporting of inter-annotator agreement (12\%). Publication bias, specification searching, and weak baselines are pervasive pitfalls that systematic reviews must account for. A tiered validation framework (Essential / Recommended / Advanced) is recommended as a benchmark for future work.

\textbf{Fourth}, geographic and linguistic coverage is critically imbalanced. Fifty-six percent of studies focus on North America, 84\% analyze English text, and no sentiment indices for Africa, Latin America, or Bangla-speaking regions (265 million speakers) are identified in the reviewed sample. This bias is not merely a sampling artifact---it has substantive implications, as sentiment forecasting dynamics in high-volatility, low-information-efficiency settings may differ fundamentally from those documented for developed economies. Recent advances in multilingual NLP (mBERT, XLM-R, BanglaBERT) make geographic expansion technically feasible, and BENI is outlined as a proposed implementation pathway.
...
\textbf{Contributions of this review}: The paper provides a methodological taxonomy of economic sentiment extraction (four method classes across 46 empirical studies), a quantitative synthesis of forecast improvements conditioned on baseline strength, a structured quality and risk-of-bias assessment covering 66 papers, explicit documentation of geographic and linguistic research gaps with a concrete expansion proposal, and a tiered validation framework for standardizing future research.

\textbf{Limitations}: The quantitative synthesis is constrained by heterogeneous reporting standards across studies---many papers omit standard errors, effect-size confidence intervals, or sufficient detail to compute them, precluding formal meta-analytic techniques such as random-effects models or funnel plot asymmetry tests. The geographic coverage analysis is limited to countries explicitly studied, which may exclude unpublished or non-English-language research. Quality scoring, while structured, was conducted by a single reviewer with partial cross-checking; independent double-coding would strengthen reliability. Finally, the review spans 2007--2025, a period of rapid methodological change, and some findings---particularly regarding LLM performance---are based on very small samples (N=2) and should be treated as preliminary.

\textbf{Priorities}: Two immediate research imperatives follow. First, \textit{geographic and linguistic expansion}---extending proven sentiment extraction methodologies to Africa, Latin America, and South Asia, particularly leveraging recently developed multilingual NLP tools (mBERT, XLM-R, MuRIL, BanglaBERT). The proposed Bangla Economic Narrative Index (BENI) illustrates this agenda: applying established techniques to a new linguistic context, validating on new economic data, and potentially discovering context-dependent narrative-economy relationships. Second, \textit{methodological standardization}---adoption of the tiered validation framework proposed here, pre-registration of sentiment forecasting studies, routine reporting of inter-annotator agreement, and standardized effect-size reporting to enable rigorous meta-analysis as the evidence base grows.
...
\noindent\textit{Coding note.} The descriptive statistics reported in the paper are calculated from the supplementary literature matrix and quality-assessment file. The workbook ``literature\_matrix.xlsx'' contains a coded 66-row matrix, an audit-summary sheet, and a coding guide. Method shares are coded from each paper's primary sentiment-extraction method; validation shares are coded from reported use of out-of-sample testing, real-time protocols, Diebold-Mariano tests, and MCS/SPA-style procedures; benchmark shares distinguish weak or unclear baselines from standard econometric/ML baselines and strong external benchmarks; forecast-performance summaries use standardized RMSE-reduction entries from the empirical-study subset where comparable effect-size information is available. Publication-bias statements are based on the coded direction of each paper's main forecasting result and should be interpreted descriptively rather than as formal funnel-plot evidence.

\subsection*{Appendix B: Risk-of-Bias and Quality Assessment}

All papers are assessed on methodological quality via three scoring dimensions. \textbf{Methodology (0--3)}: 3 = rigorous methodology with appropriate econometric or NLP techniques, clear identification strategy, and sensitivity analysis; 2 = standard methodology with minor limitations; 1 = basic methodology with notable gaps; 0 = insufficient methodological description. \textbf{Data Quality (0--2)}: 2 = large-scale, well-documented, multi-source dataset appropriate to the research question; 1 = moderate-scale or single-source data with some limitations; 0 = limited, opaque, or clearly inadequate data. \textbf{Validation (0--2)}: 2 = out-of-sample testing with real-time protocol and multiple robustness checks; 1 = basic OOS testing without real-time discipline; 0 = in-sample evaluation only or no validation. Combined scores range 0--7. Scoring was conducted by a single reviewer with cross-checking on a random 10-paper subsample ($\kappa = 0.83$). The quality assessment file ``quality\_assessment.xlsx'' contains one scored row per reviewed paper and an audit-summary sheet.

\begin{table*}[htbp]
\centering
\small
\caption{\textit{Quality Assessment Summary of 66 Reviewed Papers.} Three dimensions are scored independently; combined scores range 0--7. Quality scores have increased over time: mean 3.74 pre-2020 vs.~4.45 from 2020 onward, with the Validation dimension driving the improvement (mean 0.89 pre-2020 vs.~1.32 post-2020). Methodology and Validation scores are rounded to two decimal places; counts and percentages are exact from the supplementary quality-assessment file.}
\label{tab:quality}
\begin{tabular}{|l|c|c|}
\hline
...
The complete literature matrix (\path{literature_matrix.xlsx}) and quality assessments (\path{quality_assessment.xlsx}) are available as supplementary materials. Replication scripts for the quantitative synthesis, including standardized effect-size calculations and publication-bias diagnostics, are available upon request and will be archived publicly if the paper is accepted.

\section*{References}

\begin{thebibliography}{99}

\bibitem{Paper24} Ad\"ammer, P., \& Sch\"ussler, R. A. (2020). Forecasting the Equity Premium: Mind the News! \textit{Review of Finance}, 24(6), 1313--1355.

\bibitem{Paper28} Aguilar, P., Ghirelli, C., Pacce, M., \& Urtasun, A. (2021). Can news help measure economic sentiment? An application in COVID-19 times. \textit{Economics Letters}, 199, 109730.

\bibitem{Paper33} Aharon, D. Y., Umar, Z., Aziz, M. I. A., \& Vo, X. V. (2022). COVID-19 related media sentiment and the yield curve of G-7 economies. \textit{North American Journal of Economics and Finance}, 61, 101678.

\bibitem{Paper21} Algaba, A., Ardia, D., Bluteau, K., Borms, S., \& Boudt, K. (2020). Econometrics meets sentiment: An overview of methodology and applications. \textit{Journal of Economic Surveys}, 34(3), 512--547.

\bibitem{Paper48} Allard, M.-A., Teiletche, P., \& Zinebi, A. (2024). Enhancing Inflation Nowcasting with LLM: Sentiment Analysis on News. \textit{Working Paper}.

\bibitem{Paper30} Andre, P., Haaland, I., Roth, C., \& Wohlfart, J. (2021). Inflation Narratives. \textit{Working Paper}.

\bibitem{Paper53} Andre, P., Haaland, I., Roth, C., Wiederholt, M., \& Wohlfart, J. (2024). Narratives about the macroeconomy. \textit{SAFE Working Paper Series}, No. 426.

\bibitem{Paper3} Angeletos, G.-M., \& La'O, J. (2013). Sentiments. \textit{Econometrica}, 81(2), 739--779.

\bibitem{Paper18} Ardia, D., Bluteau, K., \& Boudt, K. (2019). Questioning the news about economic growth: Sparse forecasting using thousands of news-based sentiment values. \textit{International Journal of Forecasting}, 35(4), 1370--1386.
...
\bibitem{Paper58} Audrino, F., \& Offner, E. A. (2024). The impact of macroeconomic news sentiment on interest rates. \textit{International Review of Financial Analysis}, 94, 103293.
...
\bibitem{Paper35} Barbaglia, L., Consoli, S., \& Manzan, S. (2023). Forecasting with economic news. \textit{Journal of Business \& Economic Statistics}, 41(3), 708--719.

\bibitem{Paper49} Barbaglia, L., Consoli, S., \& Manzan, S. (2024). Forecasting GDP in Europe with Textual Data. \textit{Journal of Applied Econometrics}, 39(2), 338--355.

\bibitem{Paper54} Beiranvand, M., Malek Sadati, S. S., \& Razmi, S. M. J. (2024). Nowcasting Iran's GDP using sentiment analysis of economic news. \textit{Quarterly Journal of Applied Theories of Economics}, 11(3), 135--164.
...
\bibitem{Paper41} Bieri, M.-C. (2025). Assessing economic sentiment with newspaper text indices: evidence from Switzerland. \textit{Swiss Journal of Economics and Statistics}, 161(1), 1--60.

\bibitem{Paper55} Borja, K. J. V., Chambal, V. S., \& Wangli, D. L. (2024). Predictive Analysis of Economic Vulnerability Using News Data. \textit{SSRN Working Paper}.

\bibitem{Paper15} Bortoli, C., Combes, S., \& Renault, T. (2018). Nowcasting GDP Growth by Reading Newspapers. \textit{Economie et Statistique / Economics and Statistics}, 505--506, 17--33.

\bibitem{Paper36} Caldara, D., \& Iacoviello, M. (2022). Measuring Geopolitical Risk. \textit{American Economic Review}, 112(4), 1194--1225.

\bibitem{Paper17} Caruso, A. (2019). Macroeconomic news and market reaction: Surprise indexes meet nowcasting. \textit{International Journal of Forecasting}, 35(4), 1725--1734.

\bibitem{Paper10} Castle, J. L., Hendry, D. F., \& Martinez, A. B. (2017). Evaluating Forecasts, Narratives and Policy Using a Test of Invariance. \textit{Econometrics}, 5(3), 39.
...
\bibitem{Paper23} Clements, M. P., \& Reade, J. J. (2020). Forecasting and forecast narratives: The Bank of England Inflation Reports. \textit{International Journal of Forecasting}, 36(4), 1488--1500.

\bibitem{Paper74} Conneau, A., Khandelwal, K., Goyal, N., Chaudhary, V., Wenzek, G., Guzm\'an, F., Grave, E., Ott, M., Zettlemoyer, L., \& Stoyanov, V. (2020). Unsupervised cross-lingual representation learning at scale. \textit{Proceedings of the 58th Annual Meeting of the Association for Computational Linguistics}, 8440--8451.

\bibitem{Paper59} Cummins, R., Harris, B., \& Mahoney, N. (2024). The paradox between the macroeconomy and household sentiment. \textit{FEDS Notes}, October.

\bibitem{Paper5} Da, Z., Engelberg, J., \& Gao, P. (2015). The sum of all FEARS: Investor sentiment and asset prices. \textit{Review of Financial Studies}, 28(1), 1--32.

\bibitem{Paper70} Diebold, F. X., \& Mariano, R. S. (1995). Comparing predictive accuracy. \textit{Journal of Business \& Economic Statistics}, 13(3), 253--263.

\bibitem{Paper38} Ellingsen, J., Larsen, V. H., \& Thorsrud, L. A. (2022). News media versus FRED-MD for macroeconomic forecasting. \textit{Journal of Applied Econometrics}, 37(1), 63--81.

\bibitem{Paper50} Eugster, P., \& Uhl, M. W. (2024). Forecasting inflation using sentiment. \textit{Economics Letters}, 236, 111584.

\bibitem{Paper60} Garcia, K. N. (2024). Understanding recession response by Twitter users: A text analysis approach. \textit{Working Paper}.

\bibitem{Paper40} Gardner, B., Scotti, C., \& Vega, C. (2022). Words speak as loudly as actions: Central bank communication and the response of equity prices to macroeconomic announcements. \textit{Journal of Econometrics}, 231(2), 387--409.

\bibitem{Paper9} Gavaldon, A. A. (2017). Developing news-based Economic Policy Uncertainty index with unsupervised machine learning. \textit{Economics Letters}, 158, 47--50.

\bibitem{Paper19} Gentzkow, M., Kelly, B., \& Taddy, M. (2019). Text as data. \textit{Journal of Economic Literature}, 57(2), 535--74.

\bibitem{Paper29} Gite, S., Khatavkar, H., Kotecha, K., Srivastava, S., Maheshwari, P., \& Pandey, N. (2021). Explainable stock prices prediction from financial news articles using sentiment analysis. \textit{PeerJ Computer Science}, 7, e340.

\bibitem{Paper67} Grimmer, J., \& Stewart, B. M. (2013). Text as data: The promise and pitfalls of automatic content analysis methods for political texts. \textit{Political Analysis}, 21(3), 267--297.

\bibitem{Paper51} Gro{\ss}-Klu{\ss}mann, A. (2024). Learning deep news sentiment representations for macro-finance. \textit{Digital Finance}, 6(3), 341--377.

\bibitem{Paper2} H\'ajek, P., \& Olej, V. (2013). Evaluating Sentiment in Annual Reports for Financial Distress Prediction Using Neural Networks and Support Vector Machines. \textit{Communications in Computer and Information Science}, 384, 1--10.

\bibitem{Paper72} Hansen, P. R., Lunde, A., \& Nason, J. M. (2011). The model confidence set. \textit{Econometrica}, 79(2), 453--497.

\bibitem{Paper7} Hansen, S., \& McMahon, M. (2016). Shocking language: Understanding the macroeconomic effects of central bank communication. \textit{Journal of International Economics}, 99, S114--S133.

\bibitem{Paper68} Harvey, C. R. (2017). Presidential address: The scientific outlook in financial economics. \textit{Journal of Finance}, 72(4), 1399--1440.

\bibitem{Paper69} Harvey, D. I., Leybourne, S. J., \& Newbold, P. (1997). Testing the equality of prediction mean squared errors. \textit{International Journal of Forecasting}, 13(2), 281--291.
...
\bibitem{Paper76} Havr\'anek, T., Stanley, T. D., Doucouliagos, H., Bom, P., Geyer-Klingeberg, J., Iwasaki, I., Reed, W. R., Rost, K., \& van Aert, R. C. M. (2020). Reporting guidelines for meta-analysis in economics. \textit{Journal of Economic Surveys}, 34(3), 469--475.
...
\bibitem{Paper43} Hong, Y., Jiang, F., Meng, L., \& Xue, B. (2025). Forecasting Inflation Using Economic Narratives. \textit{Journal of Business \& Economic Statistics}, 43(1), 216--231.
...
\bibitem{Paper77} Irsova, Z., Doucouliagos, H., Havranek, T., \& Stanley, T. D. (2024). Meta-analysis of social science research: A practitioner's guide. \textit{Journal of Economic Surveys}, 38(5), 1547--1566.
...
\bibitem{Paper20} Jones, J. T., Sinclair, T. M., \& Stekler, H. O. (2020). A textual analysis of Bank of England growth forecasts. \textit{International Journal of Forecasting}, 36(4), 1478--1487.

\bibitem{Paper64} Kaczmarek, I., Iwaniak, A., Chrobak, G., \& Kazak, J. K. (2025). Integrating media sentiment with traditional economic indicators: a study on PMI, CCI, and employment during COVID-19 period in Poland. \textit{Journal of Computational Social Science}, 8(2), 1--23.
...
\bibitem{Paper44} Korinek, A. (2023). Generative AI for economic research: Use cases and implications for economists. \textit{Journal of Economic Literature}, 61(4), 1281--1317.

\bibitem{Paper6} Kr\"aussl, R., \& Mirgorodskaya, E. (2017). Media, sentiment and market performance in the long run. \textit{European Journal of Finance}, 23(11), 1059--1082.

\bibitem{Paper46} Kwon, B., Park, T., Rungcharoenkitkul, P., \& Smets, F. (2025). Parsing the pulse: Decomposing macroeconomic sentiment with LLMs. \textit{Working Paper}.

\bibitem{Paper39} Lange, K.-R., Reccius, M., Schmidt, T., M\"uller, H., Roos, M., \& Jentsch, C. (2022). Towards extracting collective economic narratives from texts. \textit{Working Paper}.
...
\bibitem{Paper25} Li, X., Wu, P., \& Wang, W. (2020). Incorporating stock prices and news sentiments for stock market prediction: A case of Hong Kong. \textit{Information Processing \& Management}, 57(5), 102212.

\bibitem{Paper45} Liapis, C. M., Karanikola, S., \& Kotsiantis, S. (2023). Investigating Deep Stock Market Forecasting with Sentiment Analysis. \textit{Entropy}, 25(2), 219.

\bibitem{Paper66} Loughran, T., \& McDonald, B. (2011). When is a liability not a liability? Textual analysis, dictionaries, and 10-Ks. \textit{Journal of Finance}, 66(1), 35--65.

\bibitem{Paper34} Lukauskas, M., Pilinkien\.{e}, V., Bruneckien\.{e}, J., Stund\v{z}ien\.{e}, A., Grybauskas, A., \& Ruzgas, T. (2022). Nowcasting with machine learning and news sentiment: The case of Lithuania. \textit{Central European Journal of Economic Modelling and Econometrics}, 14(4), 311--333.

\bibitem{Paper61} Matos, J. M. P. de. (2024). Using Twitter news sentiment analysis to forecast US GDP. \textit{Master's thesis, NOVA Information Management School, Universidade Nova de Lisboa}.
...
\bibitem{Paper12} Ren, R., Wu, D. D., \& Liu, T. (2019). Forecasting Stock Market Movement Direction Using Sentiment Analysis and Support Vector Machine. \textit{IEEE Systems Journal}, 13(1), 760--770.

\bibitem{Paper52} Rogmann, J., Beckmann, J., Gaschler, R., \& Landmann, H. (2024). Media sentiment emotions and consumer energy prices. \textit{Energy Economics}, 130, 107278.

\bibitem{Paper65} San Francisco Federal Reserve Bank. (2025). Analyzing economic sentiment and inflation expectations from the San Francisco Fed's Beige Book. \textit{Federal Reserve Bank of San Francisco Working Paper Series}.

\bibitem{Paper62} Seo, B. (2025). Econometric forecasting using ubiquitous news text: Text-enhanced factor model. \textit{International Journal of Forecasting}, 41(3), 1055--1072.

\bibitem{Paper27} Shapiro, A. H., Sudhof, M., \& Wilson, D. J. (2022). Measuring News Sentiment. \textit{Journal of Econometrics}, 228(2), 221--243.

\bibitem{Paper11} Shiller, R. J. (2017). Narrative economics. \textit{American Economic Review}, 107(4), 967--1011.

\bibitem{Paper57} Simionescu, M., \& Nicula, A. S. (2024). Sentiment Analysis as Innovation in the Inflation Forecasting in Romania. \textit{Marketing and Management of Innovations}, 15(2), 13--25.

\bibitem{Paper56} Talazadeh, S., \& Perakovi\'c, D. (2024). SARF: Stock Market Prediction with Sentiment-Augmented Random Forest. \textit{Lecture Notes of the Institute for Computer Sciences, Social Informatics and Telecommunications Engineering}, 54--64.
...
\bibitem{Paper8} Thorsrud, L. A. (2020). Words are the new numbers: A newsy coincident index of business cycles. \textit{Journal of Business \& Economic Statistics}, 38(2), 393--409.

\bibitem{Paper42} Tkacova, A., \& Gavurova, B. (2023). Economic sentiment indicators and their prediction capabilities in business cycles of EU countries. \textit{Oeconomia Copernicana}, 14(3), 977--1008.

\bibitem{Paper63} Weinig, M., \& Fritsche, U. (2025). Going viral: Inflation narratives and the macroeconomy. \textit{WiSo-HH Working Paper Series}, No. 86.

\bibitem{Paper47} Zhang, Z., Keasey, K., Lambrinoudakis, C., \& Mascia, D. V. (2024). Consumer sentiment: The influence of social media. \textit{Economics Letters}, 237, 111638.
